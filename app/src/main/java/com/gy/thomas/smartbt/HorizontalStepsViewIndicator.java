...
public class HorizontalStepsViewIndicator extends View {
    ...
    private void init() {
        ...
        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable.complete);//已经完成的icon
        mAttentionIcon = ContextCompat.getDrawable(getContext(), com.baoyachi.stepview.R.drawable.attention);//正在进行的icon
        mDefaultIcon = ContextCompat.getDrawable(getContext(), R.drawable.defualt);//未完成的icon
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
            ...
            //如果第一个正在进行,则全画虚线
            if (mStepBeanList.get(0).getState() == StepBean.STEP_CURRENT) {
